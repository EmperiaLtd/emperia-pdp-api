...
resource "aws_apigatewayv2_route" "emperia-pdp-credit-route" {
    depends_on = [
      aws_apigatewayv2_integration.emperia-pdp-integration
    ]
    api_id             = var.api_gateway_id
...
resource "aws_apigatewayv2_route" "emperia-pdp-api-get-route" {
    depends_on = [
      aws_apigatewayv2_route.emperia-pdp-credit-route
    ]
    api_id             = var.api_gateway_id
...
resource "aws_apigatewayv2_route" "emperia-pdp-redoc-route" {
    depends_on = [
      aws_apigatewayv2_route.emperia-pdp-docs-route
    ]
    api_id             = var.api_gateway_id
    route_key          = "GET /redoc"
    authorization_type = "NONE"
...
resource "aws_apigatewayv2_route" "emperia-pdp-openapi-route" {
    depends_on = [
      aws_apigatewayv2_route.emperia-pdp-redoc-route
    ]
    api_id             = var.api_gateway_id
